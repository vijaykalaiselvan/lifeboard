...
const STATUS_CONFIG = {
  todo:        { label: "To Do",       color: "bg-gray-500/20 text-gray-300",   dot: "bg-gray-400" },
  "in-progress":{ label: "In Progress", color: "bg-yellow-500/20 text-yellow-300", dot: "bg-yellow-400" },
  done:        { label: "Done",        color: "bg-green-500/20 text-green-300",  dot: "bg-green-400" },
};

const PRIORITY_CONFIG = {
  low:    { label: "Low",    color: "text-gray-400",   bar: "bg-gray-500" },
  medium: { label: "Medium", color: "text-yellow-400", bar: "bg-yellow-500" },
  high:   { label: "High",   color: "text-red-400",    bar: "bg-red-500" },
};
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 mb-5">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-3 justify-end">
              <button type="button" onClick={cancelForm} className="px-4 py-2 text-sm text-gray-400 hover:text-white">
                Cancel
              </button>
          ...
          {["all", "todo", "in-progress", "done"].map((s) => (
            <button
              ...
              className={`px-3 py-1 rounded-md text-xs font-medium transition-colors ${
                filterStatus === s ? "bg-indigo-600 text-white" : "text-gray-400 hover:text-gray-200"
              }`}
            >
        ...
        <div className="space-y-2">
          {filtered.map((task) => {
            ...
            return (
              <div
                key={task.id}
                className={`bg-gray-900 border rounded-xl px-5 py-4 flex items-start gap-4 hover:border-gray-700 transition-colors ${
              ...
              >
                ...
                <div className="flex gap-2 flex-shrink-0">
                  <button onClick={() => openEdit(task)} className="text-gray-500 hover:text-indigo-400 text-sm transition-colors">
                    Edit
                  </button>
                  <button onClick={() => handleDelete(task.id)} className="text-gray-500 hover:text-red-400 text-sm transition-colors">
                    Delete
                  </button>
                </div>
              </div>
