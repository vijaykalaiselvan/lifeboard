 ...
 * into the Lifeboard database.
 *
 * Run from the nextapp/ directory:
 *   node scripts/import-vijay.mjs
 *
 * Requires DATABASE_URL to be set in the environment.
 * If you have a .env file: node --env-file=.env scripts/import-vijay.mjs
 */
...
// ── Data from Google Sheet (K. Vijay — Personal Financial Plan) ─────────────
...
const INCOME = [
  {
    ...
    note: "Net take-home after PF & TDS deductions. Credited on 7th every month.",
  },
];
...
const DEBTS = [
  {
    ...
    currency: "INR",
    notes: "Highest rate — PAY FIRST. Extra ₹17,000/mo accelerated payment. Outstanding as of Mar 30, 2026.",
  },
  {
    name: "Car Loan",
    type: "loan",
    ...
    currency: "INR",
    notes: "EMI auto-debit via Bank of Baroda on 10th. Outstanding as of Mar 30, 2026.",
  },
  {
    name: "Housing Loan 3",
    type: "mortgage",
    ...
    currency: "INR",
    notes: "Outstanding as of Mar 30, 2026.",
  },
  {
    name: "Housing Loan 1",
    type: "mortgage",
    ...
    currency: "INR",
    notes: "Main home loan. Maintain regular EMI. Outstanding as of Mar 30, 2026.",
  },
  {
    name: "Housing Loan 2",
    type: "mortgage",
    ...
    minimumPayment: 800,
    currency: "INR",
    notes: "Small balance — closes ~Jan 2027. Outstanding as of Mar 30, 2026.",
  },
];

// Monthly recurring expenses (April 2026 baseline)
const EXPENSES = [
  ...
  { category: "Housing",   description: "House rent",                         amount: 8000,  note: "Monthly rent payment" },
  { category: "Other",     description: "Post office savings (sister marriage)", amount: 10000, note: "Stops September 2026 after sister's marriage" },
  ...
  { category: "Education", description: "Kid's school fees",                   amount: 7500,  note: "LKG from Jun 2026 — ₹90,000/yr ÷ 12" },
...
async function main() {
  ...
  if (existingIncome > 0 || existingDebts > 0 || existingExpenses > 0) {
    ...
    console.log("\nSkipping import to avoid duplicates. Delete existing records first if you want to re-import.");
    return;
  }
  ...
  console.log(`  Total debt outstanding: ₹${totalDebt.toLocaleString("en-IN")}`);
