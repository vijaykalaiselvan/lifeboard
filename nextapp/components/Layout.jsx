...
/* =============================================================
   Icons — copied verbatim from the previous Layout.jsx.
   No visual change to glyphs, only added IconMenu / IconClose
   for the mobile drawer chrome.
   ============================================================= */
...
// Mobile bottom-dock — five primary destinations chosen to keep tap targets large.
const BOTTOM_NAV = NAV.filter((n) =>
  ["/dashboard", "/finance", "/tasks", "/habits", "/goals"].includes(n.to)
);
...
export default function Layout({ children }) {
  ...
  // --- shared markup for desktop rail + mobile drawer ----------------
  function BrandRow() {
    return (
  ...
  return (
    <div className="md:flex md:h-screen bg-bg-base text-text-primary md:overflow-hidden">

      {/* ============== Desktop rail (md+) ============== */}
      <aside className="hidden md:flex w-56 flex-shrink-0 flex-col bg-bg-surface border-r border-border">
      ...
      {/* ============== Mobile top bar (< md) ============== */}
      <header className="md:hidden flex items-center justify-between gap-3 px-4 h-14 bg-bg-surface border-b border-border sticky top-0 z-30">
        <button
          onClick={() => setDrawerOpen(true)}
          className="w-10 h-10 -ml-2 flex items-center justify-center rounded-lg text-text-secondary hover:bg-bg-elevated transition-colors"
      ...
      {drawerOpen && (
        <div className="md:hidden fixed inset-0 z-40">
          ...
          />
          <aside className="absolute inset-y-0 left-0 w-72 max-w-[85vw] bg-bg-surface border-r border-border flex flex-col shadow-xl">
            <div className="flex items-center justify-between px-4 h-14 border-b border-border">
              <BrandRow />
              <button
                onClick={() => setDrawerOpen(false)}
                className="w-9 h-9 flex items-center justify-center rounded-lg text-text-secondary hover:bg-bg-elevated transition-colors"
            ...
            <ProfileSwitcher />
            <PrimaryNav onClick={() => setDrawerOpen(false)} />
            <Footer />
          </aside>
        </div>
      )}
      ...
      <main className="flex-1 md:overflow-y-auto bg-bg-base pb-20 md:pb-0 min-h-[calc(100vh-3.5rem)] md:min-h-0">
        {children}
      </main>

      {/* ============== Mobile bottom dock (< md) ============== */}
      <nav
        className="md:hidden fixed bottom-0 inset-x-0 z-30 bg-bg-surface border-t border-border flex items-stretch justify-around"
